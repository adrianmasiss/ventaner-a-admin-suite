...
const visitSchema = z.object({
  ...
  num_workers: z.number().int("El número de trabajadores debe ser entero").min(2, "Mínimo 2 trabajadores").max(50, "Máximo 50 trabajadores")
...
export const VisitForm = ({
  ...
  const fetchVisitWorkers = async (visitId: string) => {
    const {
      data,
      error
    } = await supabase.from("visit_workers").select("worker_id").eq("visit_id", visitId);
    if (!error && data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let error;
    if (editingVisit) {
      const result = await supabase.from("visits").update(data).eq("id", editingVisit.id);
      error = result.error;
      visitId = editingVisit.id;

      // Delete existing visit_workers
      if (!error) {
        await supabase.from("visit_workers").delete().eq("visit_id", editingVisit.id);
      }
    } else {
    ...
    // Insert visit_workers - Fixed payment of 20,000 per worker per visit
    if (!error && visitId) {
      const visitWorkersInserts = selectedWorkers.map(workerId => ({
        visit_id: visitId,
        worker_id: workerId,
        amount: 20000,
    ...
    if (error) {
      ...
      toast.success(editingVisit ? "Visita actualizada exitosamente" : "Visita creada exitosamente");
      resetForm();
      onSuccess();
      onClose();
    }
          ...
          <div className="space-y-2">
            ...
            <div className="border rounded-lg p-4 max-h-48 overflow-y-auto space-y-2">
              {workers.map(worker => <div key={worker.id} className="flex items-center space-x-2">
                  ...
                  <label htmlFor={worker.id} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer text-slate-950">
                    {worker.full_name}
                  </label>
                </div>)}
              {workers.length === 0 && <p className="text-sm text-muted-foreground">No hay trabajadores disponibles</p>}
            </div>
